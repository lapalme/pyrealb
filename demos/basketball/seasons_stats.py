...
# return all previous games (including current one) in the season of a given team
...
def streak(game_id, team_name, win_lose):
    n = 0
    for team in team_in_previous_games(game_id, team_name):
        if getattr(team.this_game, win_lose)().name() != team_name:
            break
        n += 1
    return n
...
def winning_streak(game_id, team_name):
    return streak(game_id, team_name, "winner")


def losing_streak(game_id, team_name):
    return streak(game_id, team_name, "loser")
...
if __name__ == "__main__":
    ...
    print(winning_streak(gid, games[gid].home().name()))
    print(losing_streak(gid, games[gid].visitors().name()))
