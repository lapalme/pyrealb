...
| ----------------------------- | --------------------------------- | ------------------------------------------------------------ |
| `FGA`<br/>`FGM`<br/>`FG_PCT`  | attempted<br/>made<br/>percentage | **F**ield **G**oal worth two points<br/>*these counts also include FG3s* |
| `FG3A`<br>`FG3M`<br>`FG3_PCT` | attempted<br>made<br>percentage   | **F**ield **G**oal worth **3** points<br/>*As they are already counted in FG, for points there are worth 1 point* |
| `FTA`<br/>`FTM`<br/>`FT_PCT`  | attempted<br/>made<br/>percentage | unopposed attempt to score points by shooting from behind the **F**ree-**T**hrow line |
| `DREB`<br/>`OREB`<br/>`TREB`  | defensive<br>offensive<br>total   | **REB**ound : awarded to a player who retrieves the ball after a missed field goal or free throw |
| `BLK`                         | block                             | occurs when a defensive player legally deflects a field goal attempt from an offensive player to prevent a score |
| `AST`                         | assist                            | attributed to a player who passes the ball to a teammate in a way that leads directly to a score by field goal |
| `STL`                         | steal                             | when a defensive player legally causes a turnover by their positive, aggressive action |
| `TOV`                         | turn-over                         | occurs when a team loses possession of the ball to the opposing team before a player takes a shot at their team's basket. |
| `PF`                          | personal foul                     | a breach of the rules that concerns illegal personal contact with an opponent |
| `PTS`                         | points                            | score in a game                                              |
| `MIN`                         | minutes                           | total number of minutes played by all players, usually 60 for a quarter (5 players each 12 minutes) |
...
| `MIN`                         | minutes                           | number of minutes on the field                               |
| `FGA`<br/>`FGM`<br/>`FG_PCT`  | attempted<br/>made<br/>percentage | **F**ield **G**oal worth two points<br/>*these counts also include FG3s* |
| `FG3A`<br>`FG3M`<br>`FG3_PCT` | attempted<br>made<br>percentage   | **F**ield **G**oal worth **3** points<br/>*As they are already counted in FG, for points there are worth 1 point* |
| `FTA`<br/>`FTM`<br/>`FT_PCT`  | attempted<br/>made<br/>percentage | unopposed attempt to score points by shooting from behind the **F**ree-**T**hrow line |
| `DREB`<br/>`OREB`<br/>`TREB`  | defensive<br>offensive<br>total   | **REB**ound : awarded to a player who retrieves the ball after a missed field goal or free throw |
...
| `TOV`                         | turn-over                         | occurs when a team loses possession of the ball to the opposing team before a player takes a shot at their team's basket. |
| `PF`                          | personal foul                     | a breach of the rules that concerns illegal personal contact with an opponent |
| `PTS`                         | points                            | used to keep track of the score in a game                    |
| `+/-`                         | plus-minus                        | change in the score when the player is either on or off the court |
| `DOUBLE`                      | kind of double                    | `double` if the player make double digits in a game in any two of the `PTS`, `TREB`, `AST`, `STL` and BLK statistics  `triple` if double digits in three statistics; `none` otherwise (most of the time...) |
...
```python
{
  ...
  "teams": {
    ...
    "summaries" : [str], # tokenized summaries
    # the following fields are undocumented
    "references": [str], # original summaries 
