...
star = Q("*")
je_decomp  = Pro("je").pe(1)
me_decomp  = Pro("me").pe(1)
mon_decomp = D("mon").pe(1).g("x")
tu_decomp  = Pro("je").pe(2).n("x")
family = [N("mère"),N("maman"),N("père"),N("papa"),N("frère"),N("soeur"),N("époux").g("x"),N("enfant").n("x")]
# pronoms paramétrables avec "maje"
moi = lambda :Pro("moi").pe(1).c("nom").maje(False)
moi_ton = lambda :Pro("moi").pe(1).tn("").maje(False)
moi_cod = lambda :moi().c("acc")
moi_coi = lambda :moi().c("dat")

vous = lambda g :Pro("moi").pe(2).g(g).c("nom")
vous_ton = lambda g :Pro("moi").pe(2).g(g).tn("")
vous_cod = lambda g :vous(g).c("acc")
vous_coi = lambda g :vous(g).c("dat")

votre = lambda :D("mon").pe(2)
mon   = lambda :D("mon").pe(1).maje(False)
# abréviations pratiques
svp        = lambda :AdvP(Adv("si"),Pro("lui").c("nom"),VP(Pro("moi").pe(2).c("dat"),V("plaire")))
# questceque = (...x)=>S(Pro("que"),VP(V("être").lier(),Pro("ce"),Pro("que"),x)).a("?")
estceque   = lambda *x : S(VP(V("être").lier(),Pro("ce"),Pro("que"),x)).a("?")

"""
à placer dans la page web de la démo d'évaluation de jsRealB
let m = ["(0)","(1)","(2)"]
let f = lambda m,g :S(vous(g),VP(V("penser"),Pro("cela"),
...
elizaInitials = [
    ...
    lambda m,g :S(VP(V("dire").t("ip").pe(2).lier(),
                moi_ton(),
                Pro("ce"),
                SP(Pro("qui"),VP(V("tracasser"),vous_cod(g))))),
    # "Is something troubling you ?"
    # Quelque chose vous préoccupe ?
    lambda m,g : S(NP(D("quelque"),N("chose"),
               VP(V("préoccuper"),vous_cod(g)))).typ({"int":"yon"})
]

au_revoir = lambda: SP(P("à"),NP(D("le"),V("revoir").t("b"))).a(".")

elizaFinals = [
    # "Goodbye.  It was nice talking to you.",
    # « Au revoir.  Ce fut un plaisir de vous parler »,
    lambda m,g : S(au_revoir(),
               SP(Pro("ce"),
                  VP(V("être").t("ps"),
                     NP(D("un"),N("plaisir"),
...
{"key":[V("excuser"),N("pardon")], "key_en":"sorry", "rank":0, "pats":[  # sorry
 {"decomp":[star], # *
  "reasmb":[
    ...
    lambda m,g : S(NP(D("le"),N("excuse").n("p")),
...
{"key":V("rappeler"), "key_en":"remember", "rank":5, "pats":[  # remember
    ...
    lambda m,g : S(PP(P("de"),Pro("quoi")),moi(),
               VP(V("souvenir").pe(1),
    ...
    lambda m,g : S(vous(g),VP(V("mentionner").t("pc"),m[2])),
    ]},
...
{"key":V("oublier"), "key_en":"forget", "rank":5, "pats":[  # forget
    ...
    lambda m,g : S(AdvP(Adv("combien"),P("de"),N("fois")),
               vous(g),
    ...
    lambda m,g :  S(vous(g),VP(V("demander"))).typ({"int":"why"}),
...
{"key":C("si"), "key_en":"if", "rank":3, "pats":[  # if
    ...
    # en: Really, if (2) ?
    # fr: Vraiment, si (2) ?
    lambda m,g : S(Adv("vraiment"),C("si"),m[2]),
    ...
    lambda m,g : S(NP(D("ce"),N("spéculation").n("p")),
               VP(V("mener").n("p"),
...
{"key":V("rêver"), "key_en":"dreamed", "rank":4, "pats":[  # dreamed
  {"decomp":[star, je_decomp,V("rêver"),P("de"),star], # * i dreamed *
   "reasmb":[
    # en: Really, (2) ?
    # fr: Réellement, (2)?
    lambda m,g : S(Adv("réellement").a(","),m[2]).a("?"),
...
{"key":Adv("peut-être"), "key_en":"perhaps", "rank":0, "pats":[  # perhaps
 {"decomp":[star], # *
  "reasmb":[
    ...
    lambda m,g : S(NP(D("ce"),N("ton"),A("incertain"))).typ({"int":"why"}),
    ...
    lambda m,g : S(vous(g),VP(V("être"),A("sûr"))).typ({"neg":True,"int":"yon"}),
    # en: Don't you know ?
    # fr: Vous ne savez pas ? 
    lambda m,g : S(vous(g),VP(V("savoir"))).typ({"neg":True}).a("?"),
    # en: How likely, would you estimate ?
    # fr: Quelle est la probabilité que vous estimez ? 
    lambda m,g : S(Pro("quel").g("f"),
...
{"key":N("nom"), "key_en":"name", "rank":15, "pats":[  # name
 {"decomp":[star], # *
  "reasmb":[
    ...
    lambda m,g : S(NP(D("le"),N("nom").n("p")),
...
{"key":Q("deutsch"), "key_en":"deutsch", "rank":0, "pats":[  # deutsch
 {"decomp":[star], # *
  "reasmb":[
    ...
    lambda m,g : S(moi(),VP(vous_coi(g),Pro("le"),V("dire").t("pc"),Adv("déjà")).a(","),
...
{"key":Q("xforeign"), "key_en":"xforeign", "rank":0, "pats":[  # xforeign
 {"decomp":[star], # *
  "reasmb":[
    ...
    lambda m,g : S(moi(),VP(V("comprendre"),NP(D("le"),N("français"))).typ({"neg":"que"})),
    ]}
]},
...
{"key":N("bonjour"), "key_en":"hello", "rank":0, "pats":[  # hello
 {"decomp":[star], # *
  "reasmb":[
    ...
    lambda m,g : S(SP(vous(g),VP(V("aller"))).typ({"int":"how"}),
    ...
    lambda m,g : S(N("bonjour").cap(True).a("."),
...
{"key":[N("ordinateur"),N("machine")], "key_en":"computer", "rank":50, "pats":[  # computer
 {"decomp":[Q("*")], # *
  "reasmb":[
    ...
    lambda m,g : S(NP(D("le"),N("ordinateur").n("p")),
...
{"key":V("être"), "key_en":"am", "rank":0, "pats":[  # am
 ...
 {"decomp":[star,[V("être").pe(2),V("être").pe(2).n("p")],tu_decomp,star], # [es|êtes] [tu|vous] # * are you *
    ...
    lambda m,g : S(Adv("peut-être"),moi(),
 ...
 {"decomp":[star,[V("être").pe(3).n("p")],star], # * sont * # * are *
  ...
  {"decomp":[star,[V("être").pe(2).t("i"),V("être").pe(2).t("i").n("p")],tu_decomp,star], # * [étais|étiez] [tu|vous] *, # * were you *
    ...
    lambda m,g : S(Pro("cela"),VP(V("suggérer"),
               SP(Pro("que"),moi(),
                  VP(V("être").t("i"),m[2])))).typ({"int":"was"}),
    # en: What do you think ?
    # fr: Qu'en pensez-vous ? 
    lambda m,g : S(vous(g),Pro("en"),
   ...
   {"decomp":[star], # *
    ...
    lambda m,g : S(vous(g),VP(V("dire"),Pro("cela"))).typ({"int":"why"}),
    # en: I don't understand that.
    # fr: Je ne comprends pas cela.
    lambda m,g : S(moi(),VP(V("comprendre"),Pro("cela"))).typ({"neg":True}),
    ]},
]},
...
{"key":D("notre"), "key_en":"your", "rank":0, "pats":[  # votre # your
 {"decomp":[star, D("notre").pe(2), star], # * your *
  "reasmb":[
    ...
    # en: Really, my (2) ?
    # fr: Vraiment, mon (2) ? 
    lambda m,g : S(Adv("vraiment"),NP(mon(),m[2])).a("?"),
    ...
    lambda m,g : S(vous(g),VP(V("vouloir"),NP(mon(),m[2]))).typ({"int":"yon"})
    ]}
]},
...
{"key":je_decomp, "key_en":"i", "rank":0, "pats":[  # je # i
 ...
 {"decomp":[star, je_decomp, V("être").pe(1), [A("triste"), A("malheureux"), V('déprimer').t("pp")], star], # * je suis @triste *# * i am* @sad *
  "reasmb":[
    ...
    lambda m,g : S(moi(),VP(V("être"),A("sûr"),
              SP(Pro("que"),Pro("lui").c("nom"),
 ...
 {"decomp":[star, je_decomp, V("être").pe(1), [A("joyeux"), A("heureux"), A("content")], star], # * i am* @happy *
  "reasmb":[
    ...
    lambda m,g : S(NP(votre(),N("traitement")),
    ...
    lambda m,g :S(vous(g),VP(V("expliquer"),
 ...
 {"decomp":[star,je_decomp, [V("penser").pe(1),V("estimer").pe(1),V("croire").pe(1)], Pro("que"),je_decomp, star ], 
  ...
  "reasmb":[
    ...
    lambda m,g : S(vous(g),VP(V("penser"),Pro("lui").c("acc"),Adv("vraiment"))).a("?"),
    # en: But you are not sure you (3).
    # fr: Mais vous n'êtes pas sûr que (2) 
    lambda m,g : S(vous(g),VP(V("être"),A("sûr"),Pro("que"),vous(g), m[2])).typ({"neg":True}),
    # en: Do you really doubt you (3) ?
    # fr: Vous doutez vraiment que (2)
    lambda m,g : S(vous(g),VP(V("douter"),Adv("vraiment"),Pro("que"),vous(g), m[2])).a("?"),
    ]},
 ...
 {"decomp":[star, je_decomp, Adv("ne"), V("vouloir").pe(1), Adv("pas"), star], 
  ...
  "reasmb":[
    ...
    lambda m,g : S(vous(g),VP(V("être"),Adv("vraiment"),A("obligé"),PP(P("de"),m[2]))
                ).typ({"int":"yon"}),
    # en: Why don't you (2) ?
    # fr: Pourquoi ne le faites-vous pas ?
    lambda m,g : S(vous(g),VP(V("faire"),Pro("lui").c("acc"))).typ({"int":"why","neg":True}),
    # en: Do you wish to be able to (2) ?
    # fr: Souhaitez-vous pouvoir (2) ?
        lambda m,g : S(vous(g),VP(V("souhaiter"),V("pouvoir").t("b"),m[2])).typ({"int":"yon"}),
    # en: Does that trouble you ?
    # fr: Cela vous gêne-t-il ? 
    lambda m,g : S(Pro("cela"),VP(V("gêner"),vous_cod(g))).typ({"int":"yon"}),
    ]},
 {"decomp":[star,je_decomp, V("sentir").pe(1), star], # * je pense * # * i feel *
  "reasmb":[
    ...
    lambda m,g : S(VP(V("parler").t("ip").pe(2).lier(),moi_ton(),PP(P("de"),NP(D("ce"),N("sentiment").n("p"))))),
    # en: Do you often feel (2) ?
    # fr: Ressentez-vous souvent (2) ?
    lambda m,g : S(vous(g),VP(V("ressentir"),Adv("souvent"),m[2])).typ({"int":"yon"}),
    # en: Do you enjoy feeling (2) ?
    # fr: Aimez-vous  (2) ?
    lambda m,g : S(vous(g),VP(V("aimer"),m[2])).typ({"int":"yon"}),
    # en: Of what does feeling (2) remind you ?
    # fr: Que vous rappelle  (2) ?
    lambda m,g : S(VP(V("rappeler"),Pro("cela"),vous_coi(g),m[2])).typ({"int":"wad"}),
    ]},
...
#     ]},
 {"decomp":[star], # *
  "reasmb":[
    # en: You say (1) ?
    # fr: Vous dites (1) ?
    lambda m,g : S(vous(g),VP(V("dire"),SP(m[1]).en("\""))).a("?"),
    # en: Can you elaborate on that ?
    # fr: Pouvez-vous élaborer sur cela?
    lambda m,g : S(vous(g),VP(V("élaborer"),PP(P("sur"),Pro("cela")))).typ({"int":"yon","mod":"poss"}),
    # en: Do you say (1) for some special reason ?
    # fr: Vous dites (1) pour une raison particulière.
    lambda m,g : S(vous(g),VP(V("dire"),SP(m[1]).en("\""), PP(P("pour"),NP(D("un"),N("raison"),A("particulier"))))),
    # en: That's quite interesting.
    # fr: C'est très intéressant.
    lambda m,g : S(Pro("ce"),VP(V("être"),AdvP(Adv("très"),A("intéressant")))),
    ]}
]},
...
{"key":Adv("oui"), "key_en":"yes", "rank":0, "pats":[  # yes
 {"decomp":[star], # *
  "reasmb":[
    ...
    # en: I see.
    # fr: Je vois
    lambda m,g : S(moi(),VP(V("voir"))),
    # en: I understand.
    # fr: Je comprends.
    lambda m,g : S(moi(),VP(V("comprendre"))),
    ]}
]},
...
{"key":N("personne"), "key_en":"no_one", "rank":0, "pats":[  # no_one
 {"decomp":[star, N("personne"), Adv("ne"), star], # * no one *
  "reasmb":[
    ...
    lambda m,g : S(Adv("sûrement"),Pro("quelqu'un"),m[2]),
...
{"key":Adv("non"), "key_en":"no", "rank":0, "pats":[  # no
  ...
  "reasmb":[
    ...
    # en: Why not ?
    # fr:Pourquoi pas ? 
    lambda m,g : S(Adv("pourquoi"),Adv("pas")).a("?"),
    # en: Why 'no' ?
    # fr: Pourquoi 'non' ?
    lambda m,g : S(Adv("pourquoi"),Adv("non").en("\"")).a("?"),
    ]}
]},
...
{"key":V("pouvoir"), "key_en":"can", "rank":0, "pats":[  # can
 {"decomp":[star, [V("pouvoir").pe(2),V("pouvoir").pe(2).n("p")], tu_decomp, star ], 
  ...
  "reasmb":[
    ...
    lambda m,g :S(Adv("peut-être"),vous(g),
                VP(V("vouloir").t("c"),
                   V("être").t("b"),
                   AP(A("capable"),
                      vous_ton(g).tn("refl"),
...
{"key":Pro("quoi"), "key_en":"what", "rank":0, "pats":[  # quoi # what
 {"decomp":[star], # *
  "reasmb":[
    ...
    lambda m,g : S(NP(D("ce"),N("question")),
    ...
    lambda m,g : S(NP(D("ce"),N("question").n("p")),
    ...
    lambda m,g : S(NP(D("quel"),N("réponse")),
               VP(V("plaire").t("c"),
                  vous_cod(g),
                  D("le"),Adv("plus"))).a("?"),
    # en: What do you think ?
    # fr: Qu'en pensez-vous ? 
    lambda m,g : S(Pro("que"),Pro("en"),
...
{"key":Adv("pourquoi"), "key_en":"why", "rank":0, "pats":[  # why
 {"decomp":[star, Adv("pourquoi"),Adv("ne"),[V("pouvoir").pe(2),V("pouvoir").pe(2).n("p")],tu_decomp,Adv("pas"), star], 
  ...
  "reasmb":[
    ...
    lambda m,g : S(Adv("peut-être"),m[2],
    ...
    # en: You want me to (2) ?
    # fr: Vous souhaitez (2) ?
    lambda m,g : S(vous(g),VP(V("souhaiter"),m[2])),
    # en: goto what
    "goto what"
    ]},
...
{"key":Pro("chacun"), "key_en":"everyone", "rank":2, "pats":[  # everyone
 {"decomp":[star], # * @everyone *
  "reasmb":[
    # en: Really, (2) ?
    # fr: Vraiment ?
    lambda m,g : S(Adv("vraiment")).a("?"),
    # en: Surely not (2).
    # fr: Sûrement pas.
    lambda m,g : S(Adv("sûrement"),Adv("pas")),
    # en: Can you think of anyone in particular ?
    # fr: Pouvez-vous penser à quelqu'un en particulier ?
    lambda m,g : S(vous(g),VP(V("penser"),
                PP(P("à"),Pro("quelqu'un"),P("en"),N("particulier")))).typ({"int":"yon"}),
    # en: Who, for example?
    # fr: Qui, par exemple ? 
    lambda m,g : S(Pro("qui").a(","),PP(P("par"),N("exemple"))).a("?"),
    # en: Are you thinking of a very special person ?
    # fr: Pensez-vous à une personne très spéciale ? 
    lambda m,g : S(vous(g),VP(V("penser"),PP(P("à"),NP(D("un"),N("personne"),Adv("très"),A("spécial"))))).typ({"int":"yon"}),
    # en: Who, may I ask ?
    # fr: Qui, si je peux me permettre ?
    lambda m,g : S(Pro("qui").a(","),SP(C("si"),moi(),VP(V("permettre"))).typ({"mod":"poss","refl":True})).a("?"),
    # en: Someone special perhaps ?
    # fr: Quelqu'un de spécial peut-être ? 
    lambda m,g : S(Pro("quelqu'un"),PP(P("de"),A("spécial")),Adv("peut-être")).a("?"),
    # en: You have a particular person in mind, don't you ?
    # fr: Vous pensez à une personne en particulier, n'est-ce pas ? 
    lambda m,g : S(vous(g),VP(V("penser"),
    ...
    # fr: De qui pensez-vous parler ? 
    lambda m,g : S(vous(g),VP(V("penser"),V("parler").t("b"),PP(P("de"),Pro("lui")))).typ({"int":"woi"}),
    ]}
]},
...
{"key":A("semblable"), "key_en":"alike", "rank":10, "pats":[  # alike
 {"decomp":[star], # *
  "reasmb":[
    # en: In what way ?
    # fr:  En quoi ?
    lambda m,g : S(P("en"),Pro("quoi")).a("?"),
    # en: What resemblence do you see ?
    # fr: Quelle ressemblance voyez-vous ?
    lambda m,g : S(NP(A("quel"),N("ressemblance")),
    ...
    lambda m,g :S(NP(D("ce"),N("ressemblance")),
              VP(V("suggérer"),Pro("cela"),vous_coi(g))).typ({"int":"wad"}),
    # en: What other connections do you see ?
    # fr: Quels autres liens voyez-vous ? 
    lambda m,g : S(NP(D("quel"),A("autre"),N("lien").n("p")),
               SP(vous(g),VP(V("voir"))).typ({"int":"yon"})),
...
def showTerms(terms):
    if type(terms)==list:
        return "["+ ", ".join(t.lemma for t in terms)+"]"
    return terms.lemma

def keywordsFrStruct():
    for kw in keywordsFr:
        print(showTerms(kw["key"]),kw["rank"],kw["key_en"])
        for pat in kw["pats"]:
            print("    %s"%(", ".join(showTerms(t) for t in pat["decomp"])))
