...
def buildSentences(nlp,text):
    lang = getLanguage()
    doc = nlp(text)
    if trace: print("End of Stanza parsing" if lang == "en" else "Analyse par Stanza terminée")
    sentences = []
    for sentence in doc.sentences:
        sentences.append(UD(UDnode_en if lang=="en" else UDnode_fr,sentence))
    return sentences
...
def process(nlp,text, n=7, showParse=False, showDiff=False):
    ...
    sentences = buildSentences(nlp,text)   # parse text to get UD for each sentence
    for sentence in sentences: # process each sentence
        ...
        try:
            ...
            pyr_expr = sentence.to_pyrealb(n==0) # create pyrealb expression
                                                 # gender and number are copied only for n==0
    ...
    if showDiff:
        print(nbDiffs,"differences",round(nbDiffs*100/len(sentences)),"%")
    print("="*10)
