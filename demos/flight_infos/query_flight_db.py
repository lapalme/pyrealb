...
def process_simple_response(intent:str) -> str:
    if intent=="greet":
        return oneOf(lambda:S(Q("hello").a(","),
                              NP(D("what"),N("information"),
                                 PP(P("about"),N("flight").n("p")),
                                 VP(V("do").pe(2),
                                    Pro("I").pe(2),
                                    V("want").t("b")))).a("?").realize(),
                     lambda:S(Q("hi").a(","),
...
def process_intent(intent:str, entity_list:list[dict[str,Any]]) -> str:
