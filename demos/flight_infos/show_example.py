...
def show_loc(prefix: str, entities:Entities, parent: str) -> [str]:
...
def show_date(entities:Entities, parent: str) -> [str]:
    return show("", entities,
                ["date_relative", "today_relative", "day_name", "month_name", "day_number", "year"],
                parent)

def show_time(entities:Entities, parent: str) -> [str]:
    res= []
...
def show_intent(intent:str, entities:Entities) -> [str]:
    ...
    def ground_fare() -> [str]:
        ...
        if len(entities)>0:
            res.append(f'for a {val if len(val)>0 else "ground transportation"} in {entities.grab_value("city_name")}')
        show_common(entities,res)
        return res
