...
def process_file(file_name):
    ...
    data = json.load(open(file_name + ".json", "r", encoding="utf-8"))
    nb = 0
    for example in data["rasa_nlu_data"]["common_examples"]:
        process_example(example)
        nb += 1
    json.dump(data, open(file_name[:-5] + ".json", "w", encoding="utf-8"), indent=2)
    print("%d examples written on file %s.json" % (nb, file_name[:-5]))
